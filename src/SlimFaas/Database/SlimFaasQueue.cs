...
public class SlimFaasQueue(IDatabaseService databaseService) : ISlimFaasQueue
{
    ...
    public async Task ListSetQueueItemStatus(string key, IList<Endpoints.QueueItemStatus> queueItemStatus) => await databaseService.ListSetQueueItemStatus($"{KeyPrefix}{key}", queueItemStatus);
