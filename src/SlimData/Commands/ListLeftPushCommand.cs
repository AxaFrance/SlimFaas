...
public struct ListLeftBinPushCommand : ISerializable<ListLeftBinPushCommand>
{
...
#pragma warning disable CA2252
    public static async ValueTask<ListLeftBinPushCommand> ReadFromAsync<TReader>(TReader reader, CancellationToken token)
#pragma warning restore CA2252
        where TReader : notnull, IAsyncBinaryReader
    {
        var key = await reader.DecodeAsync(new DecodingContext(Encoding.UTF8, false), LengthFormat.LittleEndian, token: token).ConfigureAwait(false);
        using var value = await reader.ReadAsync(LengthFormat.Compressed);
        return new ListLeftBinPushCommand
        {
            Key = key.ToString(),
